...
\todo{To by written hopefully by someone else than me.}

\subsection{Planet}

After performing all tests which were described in the previous section, we run a simulation of zombie infection on a small toroidal planet.

\subsubsection{Size}

We originally wanted to simulate a world of size $32768 \times 16384$ which equals to the surface of planet Earth.
However we had to leave this possibility because our implementation of 3D torus visualisation allows to load only texture of size up to $8192 \times 8192$.
...
In our case the ratio between major and minor radius is $ 2 : 1$.
...
The best option was to run the simulation with 32 threads on 128 nodes, which makes the world parts to be of size $512 \times 512$.

\subsubsection{Tool-chain}

During the development of the main program (referred as apocalypse), we also created many scripts of various complexity.
Here we would like to describe heir purpose and how they fit into the scheme of processing the results.
...
The time spent by waiting for MPI sends and receives was 2.252 and 1.961~ms per iteration, which means that the program waited for MPI 34.44~percent of total time.
...
In the figures of the toroidal planet, there is clearly visible a consequence of the bug discussed later in the issues section.
The visualisation started with two zombies near to each other at time 0.
