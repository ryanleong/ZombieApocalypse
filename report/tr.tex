...
\todo{Add an image of a planet here.}
...
We also performed basic scaling tests which are briefly summarized in the table (running times are in milliseconds).
...
Instead, we simulated a world of size $8192 \times 4096$; the disproportion of sizes is intetional as we definitely want to render cells as squares or close to squares.
They will be distored mostly around equators: shrinken near inner equator and widened near the outside one.
...
\todo{Document planet-1000\_times.}
...
\todo{Adam will continue here}

\subsubsection{Visualisation in time}

\todo{A few images of torus and its parts with descriptions.}
